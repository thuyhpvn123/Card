...
contract NotiStorage {
    ...
    function AddMultipleNoti(NotiParams memory params, address[] memory _to) external onlyDappOwner { 
...
contract NotiFactory is Ownable {
    ...
    constructor() Ownable(msg.sender){
        admin = 0xdf182ed5CF7D29F072C429edd8BFCf9C4151394B;
    }
    // constructor() Ownable(msg.sender){
    // }
    ...
    function subscribeUser(
        ...
        permissionManager.registerUser(user);
        mUserToHashDeviceTokens[user].push(hashDeviceToken);
        emit UserSubscribed(user, dappOwner, address(permissionManager), publicKey, encryptedDeviceToken,platform,block.timestamp,hashDeviceToken);
    }
