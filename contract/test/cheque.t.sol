...
contract SecureChequeTest is Test {
    ...
    function testPostIntentAndClaimCheque() public {
        // 1. Create and register a cheque
        address user1 = 0x9a4426D3699F627424AA3B7506D8D8aea8D7164c;
        vm.startPrank(userA);
        bytes32[] memory pubKeyHashes = new bytes32[](1);
        pubKeyHashes[0] = keccak256(abi.encodePacked(user1));
        secureCheque.registerCheques{value: 2000000000000000}(pubKeyHashes, 2000000000000000);
        ...
        uint256 claimAmount = 100000000000000;
        bytes memory sigInitial = hex"06da8bc8557989fd98902f289138098ee575e1b396eda2b92326c78335fd8e22019fdfda592ad59ab6180c372e931a03761d68b792e6d50842a2dbb23bf925e200";
        ...
        // Check if balance was updated
        uint256 balance = secureCheque.balances(user1);
        assertEq(balance, claimAmount, "Balance was not updated correctly");
        
        // Check if remaining amount was refunded to cheque creator
        uint256 creatorBalance = secureCheque.balances(userA);
        assertEq(creatorBalance, 1900000000000000, "Remaining amount was not refunded to creator");
        
        vm.stopPrank();
    }

    function GetByteCode() public pure{
        //registerCheques
        address user1 = 0x9a4426D3699F627424AA3B7506D8D8aea8D7164c;
        ...
        bytes memory bytesCodeCall = abi.encodeCall(
            SecureCheque.registerCheques,
            (pubKeyHashes,amount)
        );
        console.log("SecureCheque registerCheques: ");
        ...
        //
        uint256 claimAmount = 100000000000000;
        bytes memory sigInitial = hex"a90ad3aefd9e80ab1d6eb873e8bbc4c2cdf34457ef48def261a2fa91a7b632925bab0e060479fe5c9a697dbe15b222d40dbb47f107b15a4ed9459a9d14c8202e00";
        bytes32 initialSigHash = keccak256(sigInitial);
        bytesCodeCall = abi.encodeCall(
            SecureCheque.postClaimIntent,
            (initialSigHash)
        );
        console.log("SecureCheque postClaimIntent: ");
        console.logBytes(bytesCodeCall);
        console.log(
            "-----------------------------------------------------------------------------"
        );
        ...
        SecureCheque.TransferStep[] memory emptySteps = new SecureCheque.TransferStep[](0);
        bytesCodeCall = abi.encodeCall(
            SecureCheque.claimCheque,
            (emptySteps, claimAmount, sigInitial)
        );
        console.log("SecureCheque claimCheque: ");
        console.logBytes(bytesCodeCall);
        console.log(
            "-----------------------------------------------------------------------------"
        );
